var tableData = [
  ...
  function pageButtons(pages) {
    ...
    for (var page = maxLeft; page <= maxRight; page++) {
      wrapper.innerHTML += `<button value=${page} class="page page-link btn ">${page}</button>`
    }
  
    if (state.page != 1) {
      wrapper.innerHTML = `<button value=${1} class="page page-link btn">&#171; First</button>` + wrapper.innerHTML
    }
  
    if (state.page != pages) {
      wrapper.innerHTML += `<button value=${pages} class="page page-link btn">Last &#187;</button>`
    }
